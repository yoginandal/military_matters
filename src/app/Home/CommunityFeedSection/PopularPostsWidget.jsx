...
export function PopularPostsWidget() {
  return (
    <div className="bg-white dark:bg-neutral-900 border-2 border-slate-300 dark:border-white/10 rounded-2xl p-6">
        ...
        {popularPosts.map((item) => (
          <div key={item.id} className="flex gap-4 group cursor-pointer">
            ...
            <div className="flex flex-col justify-center">
              <h5 className="text-sm font-bold text-slate-700 dark:text-slate-200 leading-tight group-hover:text-orange-500 transition-colors line-clamp-2">
                {item.title}
              </h5>
              <span className="text-[10px] text-slate-600 dark:text-slate-500 font-mono mt-1 block">
                READ IN 4 MIN
              </span>
            </div>
          </div>
      ...
      <button className="w-full mt-6 py-3 rounded-xl border border-slate-300 dark:border-white/10 bg-slate-100 dark:bg-white/5 text-xs font-bold text-slate-900 dark:text-white uppercase tracking-widest hover:bg-orange-500 hover:border-orange-500 hover:text-white dark:hover:text-black transition-colors flex items-center justify-center gap-2 group">
        View All Reports
