...
const TopBar = () => {
  ...
  return (
    <aside
      className="hidden w-full px-5 sm:px-8 py-2 md:block border-b border-white/10"
      aria-label="Top bar"
    >
      <div className="flex flex-wrap items-center justify-between mx-auto text-sm text-white">
            ...
            <div className="flex gap-1" role="list">
              {socialIcons.map((item, index) => (
                <a
                  ...
                  role="listitem"
                  className="p-2 rounded-full text-slate-100 transition-all duration-300 hover:text-amber-400 hover:bg-white/10"
                >
        ...
        >
          <div className="flex items-center gap-4 bg-black/20 px-4 py-2 rounded-full border border-white/10 shadow-inner w-full">
            <div
              className="h-2.5 w-2.5 bg-amber-400 rounded-full animate-pulse shadow-[0_0_8px_theme('colors.amber.400')]"
              aria-hidden="true"
            ></div>
            <div className="overflow-hidden relative h-6 flex-1">
              {" "}
              {/* Increased height */}
              <div className="animate-scroll whitespace-nowrap font-bold tracking-wider text-white absolute inset-y-0 flex items-center">
        ...
        <nav aria-label="Quick actions">
          <Link
            ...
            className="px-6 py-2.5 text-sm font-bold uppercase tracking-wider text-slate-900 bg-gradient-to-r from-amber-400 to-amber-500 rounded-md shadow-lg transition-all duration-300 hover:scale-105 hover:shadow-amber-400/50 focus:outline-none focus:ring-4 focus:ring-amber-400 focus:ring-offset-2 focus:ring-offset-slate-900"
          >
            Contact HQ
          </Link>
        </nav>
